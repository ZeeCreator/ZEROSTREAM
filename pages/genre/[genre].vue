<template>
  <div class="px-margin-mobile md:px-margin-desktop max-w-container-max mx-auto py-8">
    <h1 class="font-headline-lg text-headline-lg capitalize mb-8">{{ genre }}</h1>
    <div v-if="pending && !genreItems.length" class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-gutter">
    ...
    <div v-else-if="!genreItems.length" class="text-center py-20">
      <span class="material-symbols-outlined text-6xl text-on-surface-variant/30 mb-4">category</span>
      <p class="text-on-surface-variant font-body-lg">Tidak ada konten untuk genre ini</p>
    </div>
    <template v-else>
      <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-gutter">
        <NuxtLink
          v-for="(item, i) in genreItems"
          :key="item.id || i"
      ...
      <div class="flex items-center justify-center gap-4 mt-10">
        <NuxtLink
          :to="currentPage > 1 ? `/genre/${genre}?page=${currentPage - 1}` : undefined"
        ...
        <NuxtLink
          :to="`/genre/${genre}?page=${currentPage + 1}`"
          class="w-10 h-10 rounded-full flex items-center justify-center transition-all bg-surface-container-highest text-on-surface hover:bg-primary hover:text-on-primary"
        >
...
const { data: genreData, pending } = useAsyncData<PaginatedResponse>('genre-' + genre + '-' + currentPage.value, () =>
  $fetch<PaginatedResponse>(`/api/genre/${genre}?page=${currentPage.value}`, {
...
const genreItems = computed(() => genreData.value?.items || [])

useHead({
  title: computed(() => `${genre.charAt(0).toUpperCase() + genre.slice(1)} - ZeroStream`)
})
</script>
