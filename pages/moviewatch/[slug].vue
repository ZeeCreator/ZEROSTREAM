  ...
  <div v-else-if="detail" class="pb-section-gap">
        ...
        <div class="flex flex-col md:flex-row gap-8 items-start">
          ...
          <div class="flex-1 space-y-4">
            <div class="flex items-center gap-4">
              <span v-if="detail.quality" class="bg-secondary-container/20 text-secondary text-label-caps px-3 py-1 rounded-sm border border-secondary/30">
                {{ detail.quality }}
              </span>
            ...
            <div class="flex flex-wrap items-center gap-x-6 gap-y-2 text-on-surface-variant font-label-md text-label-md">
              ...
              <span class="w-1 h-1 bg-outline rounded-full"></span>
              <span class="text-secondary border border-secondary/30 px-2 py-0.5 rounded">{{ detail.quality || 'HD' }}</span>
              <div class="flex gap-2 ml-2">
                <span
                  v-for="g in detail.genres"
                  :key="g"
                  class="bg-surface-container-highest px-3 py-1 rounded-full text-xs"
                >
                  {{ g }}
                </span>
              </div>
    ...
    <main class="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop py-section-gap grid grid-cols-1 lg:grid-cols-12 gap-12">
      ...
      <div class="lg:col-span-8 space-y-10">
        ...
        <section v-if="activeTab === 'player' && selectedSource">
          <div class="relative aspect-video rounded-xl overflow-hidden bg-black shadow-2xl mb-6">
            <iframe
        ...
        <!-- Server Selection -->
        <section ref="serverSection">
          <h2 class="font-headline-md text-headline-md text-primary mb-6 flex items-center gap-3">
            <span class="w-8 h-[2px] bg-primary"></span>
            Pilih Server
          </h2>
          <div v-if="watchSources.length" class="flex flex-wrap gap-3">
            <button
          ...
          <p v-else class="text-on-surface-variant font-body-md">Tidak ada server tersedia.</p>
        </section>

        <!-- Download Links -->
        <section v-if="detail.downloadLinks?.length">
          <h2 class="font-headline-md text-headline-md text-primary mb-6 flex items-center gap-3">
            <span class="w-8 h-[2px] bg-primary"></span>
            Download
          </h2>
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <a
              v-for="(link, i) in detail.downloadLinks"
              :key="i"
              :href="link.url"
              target="_blank"
              rel="noopener"
              class="glass-panel px-5 py-3 rounded-xl flex items-center gap-3 hover:bg-primary/10 hover:border-primary/30 border border-outline-variant/20 transition-all group"
            >
              <span class="material-symbols-outlined text-secondary">download</span>
              <div class="flex-1">
                <p class="font-label-md text-label-md text-on-surface group-hover:text-primary transition-colors truncate">{{ link.provider }}</p>
                <p class="text-xs text-on-surface-variant">Download Link</p>
              </div>
              <span class="material-symbols-outlined text-on-surface-variant group-hover:text-primary">open_in_new</span>
            </a>
          </div>
        </section>
      </div>
          ...
          <div class="space-y-4">
            ...
            <div class="flex items-center justify-between border-b border-outline-variant/20 pb-3">
              <div class="flex items-center gap-3">
                ...
                <span class="text-on-surface-variant font-label-md text-label-md">Kualitas</span>
              </div>
              <span class="text-secondary font-bold">{{ detail.quality || 'HD' }}</span>
            </div>
            ...
            <div class="pt-2">
              ...
              <div class="flex flex-wrap gap-2">
                <span
                  v-for="g in detail.genres"
                  :key="g"
                  class="bg-surface-container-highest px-3 py-1 rounded-full text-xs text-on-surface-variant border border-outline-variant/20"
                >
                  {{ g }}
                </span>
              </div>
...
import type { MovieDetailResponse, WatchResponse, WatchSource } from '~/types'
import CustomPlayer from '~/components/player/CustomPlayer.vue'

const route = useRoute()
const slug = route.params.slug as string
const activeTab = ref<'info' | 'player'>('info')
const selectedSource = ref<WatchSource | null>(null)

const { data: movieRes, pending } = useAsyncData<MovieDetailResponse>('moviewatch-' + slug, () =>
  $fetch<MovieDetailResponse>(`/api/moviewatch/${slug}`, {
...
const { data: watchRes } = useAsyncData<WatchResponse>('watchsrc-' + slug, () =>
  $fetch<WatchResponse>(`/api/watch-sources/${slug}`, {
    headers: { Accept: 'application/json' }
  }),
  {
    default: () => null,
...
const detail = computed(() => movieRes.value?.data || null)
const watchSources = computed(() => watchRes.value?.data || [])

const serverSection = ref<HTMLElement | null>(null)

const scrollToServer = () => {
  activeTab.value = 'player'
  nextTick(() => {
    serverSection.value?.scrollIntoView({ behavior: 'smooth', block: 'start' })
  })
}
...
onMounted(() => {
  if (watchSources.value.length && !selectedSource.value) {
    selectSource(watchSources.value[0])
  }
})

useHead({
  title: computed(() => detail.value ? `${detail.value.title} - ZeroStream` : 'Movie - ZeroStream')
})
</script>
